...
// caso não exista, procurar no localStorage se existe uma página salva.
    ...
    $("#booklet .paginas .pagina").addClass("closed");
...
function hover () { // Altera o source de uma imagem no mouseover. Imagens dentro de botões/links/divs também. Basta colocar o data-hover no botão, link, div ou imagem.

  $("button[data-hover]:not([data-hover='']), a[data-hover]:not([data-hover='']), div[data-hover]:not([data-hover=''])").mouseover(function (){
    var img_hover = $(this).attr("data-hover");
    var img_src = $("img:nth-child(1)", this).attr("src");
    $("img:nth-child(1)", this).attr("src", img_hover);
    $(this).attr("data-hover", img_src);
    
  });

  $("button[data-hover]:not([data-hover='']), a[data-hover]:not([data-hover='']), div[data-hover]:not([data-hover=''])").mouseout(function (){
    var img_hover = $(this).attr("data-hover");
    var img_src = $("img:nth-child(1)", this).attr("src");
    $("img:nth-child(1)", this).attr("src", img_hover);
    $(this).attr("data-hover", img_src);
  ...
  $("img[data-hover]:not([data-hover=''])").mouseover(function (){
    var img_hover = $(this).attr("data-hover");
  ...
  $("img[data-hover]:not([data-hover=''])").mouseout(function (){
    var img_hover = $(this).attr("data-hover");
    var img_src = $(this).attr("src");
    $(this).attr("src", img_hover);
    $(this).attr("data-hover", img_src);
...
function goTo( i, bool ){
  ...
  if(bool){
      ...
      $("#booklet .paginas .pagina").removeClass("open");
      $("#booklet .paginas .pagina").addClass("closed");
      $('#booklet').animate({
        scrollTop: 0
    }, 2000);
      ...
      $("#booklet .paginas .pagina:nth-child(" + ap + ")").addClass("open");
      $("#booklet .paginas .pagina:nth-child(" + ap + ")").removeClass("closed");
      history.pushState( { foo: "page"+i }, "Exemplo", "?page="+i);
      savePage(ap);
    }
}
